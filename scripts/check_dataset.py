...
def check_out_of_bounds_boxes(
    ...
    for ann_file in Path(annotations_dir).glob("*.txt"):
        ...
        for idx, (x, y, w, h, _class_id) in enumerate(boxes):
    ...
    for ann_file in Path(annotations_dir).glob("*.txt"):
        ...
        for _, _, _, _, class_id in boxes:
            if class_id not in valid_classes:
                invalid_classes.append(int(class_id))
...
Examples:
  ...
  python scripts/check_dataset.py -a data/annotations -i data/images --verbose
    ...
    parser.add_argument(
        "-i", "--images_dir", required=True, help="Path to directory containing image files"
    )
    ...
    oob_issues = check_out_of_bounds_boxes(args.annotations_dir)
    if oob_issues:
