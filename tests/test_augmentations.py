...
def test_augmentation_preserves_box_count():
    ...
    boxes = np.array(
        [[0.25, 0.3, 0.45, 0.5, 1], [0.5, 0.4, 0.7, 0.6, 2], [0.6, 0.2, 0.85, 0.45, 3]],
        dtype=np.float32,
    )
    ...
    try:
        augmented = aug(image=image, bboxes=boxes[:, :4], labels=boxes[:, 4].astype(int))
...
def test_augmentation_boxes_stay_within_bounds():
    ...
    boxes = np.array([[0.25, 0.3, 0.45, 0.5, 1], [0.5, 0.5, 0.7, 0.7, 2]], dtype=np.float32)
...
def test_augmentation_preserves_class_ids():
    ...
    boxes = np.array([[0.25, 0.3, 0.45, 0.5, 5], [0.5, 0.4, 0.7, 0.6, 7]], dtype=np.float32)
...
def test_augmentation_output_shape():
    ...
    boxes = np.array([[0.25, 0.3, 0.45, 0.5, 1]], dtype=np.float32)
